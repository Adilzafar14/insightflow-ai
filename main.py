...
import hashlib, hmac, secrets, sqlite3, io
...
def process_education(df):
    ...
    if att_col:
        ...
        kpis["Low Attendance (<75%)"] = int((df[att_col] < 75).sum())
    ...
    la  = kpis.get("Low Attendance (<75%)", 0)
    ...
    if at: insights.append(("warn" if at < 75 else "good", f"{'⚠️' if at<75 else '✅'} Avg Attendance: **{at:.1f}%** | {la} students below 75% {'— Critical!' if at<65 else ''}"))
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        # Navigation
        if is_admin():
            pages = {"upload": "📁  Upload & Analyze", "clients": "👥  Clients", "users": "🔐  Users", "festival": "🎉  Festivals"}
        else:
            pages = {"dashboard": "📈  My Dashboard"}
